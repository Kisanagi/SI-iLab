          ...
          <div className="max-w-3xl mx-auto">
            <div className="flex items-center gap-2 bg-blue-50 border border-blue-200 rounded-xl px-3 py-2 w-fit max-w-full">
              {attachedFile.preview ? (
                <img src={attachedFile.preview} alt="preview" className="h-10 w-10 object-cover rounded-lg shrink-0" />
              ) : (
                <div className="h-10 w-10 bg-red-100 rounded-lg flex items-center justify-center shrink-0">
                  <span className="text-xs font-bold text-red-500">PDF</span>
                </div>
              )}
              <span className="text-xs text-gray-700 truncate max-w-[180px]">{attachedFile.name}</span>
              <button onClick={removeAttachedFile} className="text-gray-400 hover:text-gray-600 shrink-0 ml-1 text-sm">✕</button>
            </div>
          </div>
        </div>
      )}
